...
public class ShooterSubsystem extends SubsystemBase {
  ...
  HashMap<Double, Rotation2d> distanceToAngleMap = new HashMap<>() {
    {
  ...
  public Rotation2d getRotationFromDistance(double distance) {
    ...
    for (double distKey : distanceToAngleMap.keySet()) {
      // Sets the minimum
      if (distance > distKey && distKey > maxBelow)
        maxBelow = distKey;

      // Sets the maximum
      if (distance < distKey && distKey < minAbove)
        minAbove = distKey;
    ...
    Rotation2d angleDelta = distanceToAngleMap.get(minAbove).minus(distanceToAngleMap.get(maxBelow));
