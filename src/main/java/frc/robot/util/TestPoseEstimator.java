...
public class TestPoseEstimator {
  ...
  private final TimeInterpolatableBuffer<Pose2d> poseBuffer = TimeInterpolatableBuffer.createBuffer(BUFFER_DURATION);
  ...
  public void updateWithTime(
    ...
    if (gyroAngle.isPresent()) {
      twist = new Twist2d(
          twist.dx,
          twist.dy,
          gyroAngle.get().minus(lastGyroAngle).getRadians());
   ...
   * calling {@link PoseEstimator#update} every loop.
   *
   * <p>
   ...
   *                              since FPGA startup (i.e., the epoch of this
   ...
   * {@link PoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
   *
